...
export default async function handler(req, res) {
  ...
  // Try in-memory first, fall back to cookie token
  const session = getShopToken(shop)
  const accessToken = session?.accessToken || getAccessToken(req)
  if (!accessToken) return res.status(401).json({ error: 'Not installed - please reconnect at /install' })
  ...
  try {
    ...
    const enriched = (products||[]).map(p => {
      ...
      return { id:p.id, title:p.title, productType:p.product_type, image:p.image?.src, totalInventory, variantCount:variants.length, status, sizes, colors, sellingOut }
    })
